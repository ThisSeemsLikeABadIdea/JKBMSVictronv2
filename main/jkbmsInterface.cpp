...
uart_port_t portNum = UART_PORT_NUM;
...
// Battery Capacity
uint8_t raw_battery_remaining_capacity =0;
uint32_t raw_total_battery_capacity = 0;
float batCycles =0;
float batTotalCycles = 0;
...
extern "C" {
  uint8_t get_bat_rem_cap(){
    return raw_battery_remaining_capacity;
  }
  float get_bat_cycles()
  {
    return batCycles;
  }
float get_bat_total_cycles()
{
  return batTotalCycles;
}
    ...
    uint32_t get_total_cap()
    {
      return raw_total_battery_capacity;
    }
}
...
bool parse_packet(const std::vector<uint8_t>& buffer) {
  ...
  //dumpVectorAsHex(buffer);
  uint16_t offset = buffer[1] + 3;
  //printf("\n current offset is %i", offset);
  raw_battery_remaining_capacity = buffer[offset + 3 * 5];
  //printf("\n current rawCap is %i", raw_battery_remaining_capacity);
  batCycles =(float) jk_get_16bit(offset + 4 + 3 * 5);
  batTotalCycles = (float) jk_get_32bit(offset + 4 + 3 * 6);
  raw_total_battery_capacity = jk_get_32bit(offset + 10 + 3 * 36);
  //printf("\n current total Cap is %li", raw_total_battery_capacity);
...
void uartRxHandler(void *arg) {
    ...
    if (uart_driver_install(portNum, rx_buffer_size, rx_buffer_size, 10, &uart_queue, 0) == ESP_OK) {
        ...
        while (1) {
            ...
            if (xQueueReceive(uart_queue, (void *)&event, (TickType_t)200)) {
...
void JKBMS::start(){
    ...
    uart_set_pin(UART_NUM_0, JK_TX_PIN, JK_RX_PIN, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);    
    // Create a task to handle UART events    
    xTaskCreate(uartRxHandler, "uart_rx_handler", 4096, NULL, 10, NULL);  
    const int uart_buffer_size = (1024 * 2);
    
    QueueHandle_t uart_queue;    
    esp_err_t uart_install_status = uart_driver_install(UART_NUM_0, uart_buffer_size, uart_buffer_size, 10, &uart_queue, 0);
